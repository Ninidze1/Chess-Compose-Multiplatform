...
import com.ninidze.chesscomposekmm.domain.engine.evaluation.CompleteEvaluator
import com.ninidze.chesscomposekmm.domain.engine.evaluation.Evaluator
import com.ninidze.chesscomposekmm.domain.engine.evaluation.SimplifiedEvaluator
import com.ninidze.chesscomposekmm.domain.engine.log.Logger
import com.ninidze.chesscomposekmm.domain.engine.tt.TranspositionTable
 ...
 */
open class SearchEngine(private var config: Config) : KoinComponent {
    ...
    val board: com.ninidze.chesscomposekmm.domain.engine.Board =
        com.ninidze.chesscomposekmm.domain.engine.Board()
    private var observer: SearchObserver? = null
    private lateinit var evaluator: Evaluator
    private lateinit var tt: TranspositionTable
    var nodes: Array<Node> = Array(MAX_DEPTH) { i -> Node(this, i) }
    var history: Array<ShortArray> = Array(6) { ShortArray(64) } // By piece type and destiny square

    private var bestMoveScore: Int = 0
    var bestMove: Int = 0
        private set
    ...
    init {
        ...
        for (i in 1..63) {
            for (j in 1..63) {
                logMatrix[i][j] =
                    (ln(i.toDouble()) * ln(j.toDouble()) / (ln(63.0) * ln(63.0))).toFloat()
            }
        }
    ...
    private fun clearHistory() {
        for (i in 0..5) {
            platformUtils.arrayFill(history[i], 0.toShort())
        }
    }
     ...
     */
    private fun evaluate(node: Node, foundTT: Boolean) {
        ttEvalProbe++
        ...
        tt[board, TranspositionTable.TYPE_EVAL, node.distanceToInitialPly, 0, Move.NONE, 0, node.staticEval] =
            false
    }

    private fun refineEval(node: Node, foundTT: Boolean): Int {
    ...
    private fun quiescentSearch(qsdepth: Int, alphaIn: Int, betaIn: Int): Int {
        var alpha = alphaIn
        var beta = betaIn
        ...
        val ttDepth =
            if (generateChecks || checkEvasion) TranspositionTable.DEPTH_QS_CHECKS else TranspositionTable.DEPTH_QS_NO_CHECKS
        ...
        if (!checkEvasion) {
            ...
            if (bestScore >= beta) {
                if (!foundTT) {
                    tt[board, TranspositionTable.TYPE_FAIL_HIGH, distanceToInitialPly, TranspositionTable.DEPTH_QS_CHECKS, Move.NONE, bestScore, node.staticEval] =
                        false
                }
                return bestScore
            }
        ...
        node.moveIterator.genMoves(
            node.ttMove,
            if (generateChecks) MoveIterator.GENERATE_CAPTURES_PROMOS_CHECKS else MoveIterator.GENERATE_CAPTURES_PROMOS
        )
        ...
        while (true) {
            ...
            if (!node.moveIterator.checkEvasion
                ...
                val futilityValue =
                    futilityBase + Evaluator.PIECE_VALUES[Move.getPieceCaptured(board, node.move)]
        ...
        tt[board, if (bestScore <= alpha)
            ...
            TranspositionTable.TYPE_EXACT_SCORE, distanceToInitialPly, ttDepth, bestMove, bestScore, node.staticEval] =
            false
    ...
    fun search(
        ...
        if (nodeType != NODE_ROOT && bestMove != Move.NONE && (platformUtils.currentTimeMillis() > thinkToTime || nodeCount > thinkToNodes)) {
        ...
        if (!checkEvasion) {
            ...
            val evalTT = if (excludedMove == Move.NONE || foundTT) foundTT else tt.search(
                board,
                distanceToInitialPly,
                false
            )
        ...
        if (!checkEvasion && allowPrePruning) {
            ...
            if (nodeType == NODE_NULL
                ...
                && board.pawns and (board.whites and BitboardUtils.R7 or (board.blacks and BitboardUtils.R2)) == 0L
            ) { // No pawns on 7TH
                razoringProbe++
            ...
            if (nodeType == NODE_NULL
                ...
                board.doMove(Move.NULL_1, false, false)
        ...
        while (true) {
            ...
            if (nodeType != NODE_ROOT
                ...
                val excScore =
                    search(nodeType, depthRemaining shr 1, seBeta - 1, seBeta, false, node.move)
                if (excScore < seBeta) {
            ...
            if (nodeType != NODE_ROOT
                ...
                if (depthRemaining >= LMR_DEPTHS_NOT_REDUCED) {
                    reduction =
                        (0.5f + 4.4f * (if (nodeType == NODE_NULL) 1f else 0.85f) * (1f - 0.8f * nodeEvalDiff / (NODE_EVAL_DIFF_MAX - NODE_EVAL_DIFF_MIN)) // [-0.5..0.5]
                                * (1f - 1.1f * node.moveIterator.lastMoveScore / (HISTORY_MAX - HISTORY_MIN)) * logMatrix[min(
                            depthRemaining / PLY,
                            63
                        )][min(moveCount, 63)]).toInt()
                ...
                if (bestMove != Move.NONE) { // There is a best move
                    ...
                    if (newDepth - reduction < FUTILITY_MARGIN_PARENT.size) {
                        val futilityValue =
                            node.staticEval + FUTILITY_MARGIN_PARENT[newDepth - reduction]
                        if (futilityValue <= alpha) {
                            futilityHit++
            ...
            if (score > bestScore && (config.rand == 0 //... insert errors to lower the ELO
                        ...
                        || platformUtils.randomInt(1000) > config.rand)
            ) {
                bestMove = node.move
                bestScore = score
        ...
        tt[board, if (bestScore <= alpha)
            ...
            TranspositionTable.TYPE_EXACT_SCORE, distanceToInitialPly, depthRemaining, bestMove, bestScore, node.staticEval] =
            excludedMove != Move.NONE
    ...
    private fun notifyMoveFound(move: Int, score: Int, alpha: Int, beta: Int) {
        val time = platformUtils.currentTimeMillis()
     ...
     */
    open fun go(searchParameters: SearchParameters = SearchParameters()) {
    ...
    private fun prepareRun() {
        ...
        if (config.useBook && board.isUsingBook
            && (config.bookKnowledge == 100 || platformUtils.randomFloat() * 100 < config.bookKnowledge)
        ) {
    ...
    private fun runStepped() {
        ...
        var alpha =
            if (initialScore - aspWindows!![failLowCount] > -Evaluator.MATE) initialScore - aspWindows!![failLowCount] else -Evaluator.MATE
        var beta =
            if (initialScore + aspWindows!![failHighCount] < Evaluator.MATE) initialScore + aspWindows!![failHighCount] else Evaluator.MATE
        ...
        while (true) {
            ...
            if (rootScore <= alpha) {
                failLowCount++
                alpha =
                    if (failLowCount < aspWindows!!.size && initialScore - aspWindows!![failLowCount] > -Evaluator.MATE)
                ...
                failHighCount++
                beta =
                    if (failHighCount < aspWindows!!.size && initialScore + aspWindows!![failHighCount] < Evaluator.MATE)
    ...
    private fun run() {
        ...
        val bestMove: Int = this.bestMove
        val ponderMove: Int = globalPonderMove
     ...
     */
    private fun setInitialSearchParameters(searchParameters: SearchParameters) {
     ...
     */
    private fun updateSearchParameters(searchParameters: SearchParameters) {
    ...
    private fun evaluateDraw(distanceToInitialPly: Int): Int {
     ...
     */
    private fun historyGood(node: Node, move: Int, depth: Int) {
    ...
    private fun historyBad(move: Int, depth: Int) {
        if (move == Move.NONE || Move.isTactical(move)) {
            return
        }
    ...
    companion object {
        ...
        private const val NODE_PV = 1
        private const val NODE_NULL = 2
        private const val PLY = 1

        const val HISTORY_MAX = Short.MAX_VALUE - 1
        const val HISTORY_MIN = Short.MIN_VALUE + 1

        // The bigger the treshold, the more moves are pruned
        private val HISTORY_PRUNING_TRESHOLD = intArrayOf(7000, 5994, 5087, 5724)

        private const val LMR_DEPTHS_NOT_REDUCED = 3 * PLY
        private val SINGULAR_MOVE_DEPTH = intArrayOf(0, 6 * PLY, 8 * PLY) // By node type
        private val IID_DEPTH = intArrayOf(5 * PLY, 5 * PLY, 8 * PLY)

        private const val IID_MARGIN = 150
        private const val SINGULAR_EXTENSION_MARGIN_PER_PLY = 1
        private val ASPIRATION_WINDOW_SIZES = intArrayOf(10, 25, 150, 400, 550, 1025)
        private const val FUTILITY_MARGIN_QS = 50
        ...
        private const val NODE_EVAL_DIFF_MIN = -250
        private const val NODE_EVAL_DIFF_MAX = 250
