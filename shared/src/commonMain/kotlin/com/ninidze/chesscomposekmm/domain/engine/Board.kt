...
import com.ninidze.chesscomposekmm.domain.engine.hash.ZobristKey
import com.ninidze.chesscomposekmm.domain.engine.movegen.LegalMoveGenerator
...
class Board : KoinComponent {
    ...
    private var legalMoveGenerator = LegalMoveGenerator()
    internal var legalMoves = IntArray(256)
    internal var legalMoveCount = -1 // if -1 then legal moves not generated
    private var legalMovesKey = longArrayOf(0, 0)
    private var movesSan = HashMap<Int, String>()
    ...
    private var fiftyMovesRule = 0
    var initialMoveNumber = 0
    var moveNumber = 0
    var outBookMove = Int.MAX_VALUE
    private var key = longArrayOf(0, 0)

    lateinit var initialFen: String

    // History array indexed by moveNumber
    private var keyHistory = Array(MAX_MOVES) { LongArray(2) } // to detect draw by treefold
    private var moveHistory = IntArray(MAX_MOVES)
    private var whitesHistory = LongArray(MAX_MOVES)
    private var blacksHistory = LongArray(MAX_MOVES)
    private var pawnsHistory = LongArray(MAX_MOVES)
    private var rooksHistory = LongArray(MAX_MOVES)
    private var queensHistory = LongArray(MAX_MOVES)
    private var bishopsHistory = LongArray(MAX_MOVES)
    private var knightsHistory = LongArray(MAX_MOVES)
    private var kingsHistory = LongArray(MAX_MOVES)
    private var flagsHistory = LongArray(MAX_MOVES)
    private var fiftyMovesRuleHistory = IntArray(MAX_MOVES)
    private var seeGain = IntArray(32)
    ...
    private var bbAttacks = BitboardAttacks.getInstance()
    ...
    fun canCastleKingSide(color: Int, ai: AttacksInfo): Long {
        if (if (color == Color.W) whiteKingsideCastling else blackKingsideCastling) {
            ...
            val rookRoute =
                BitboardUtils.getHorizontalLine(rookDestiny, rookOrigin) and rookOrigin.inv()
            ...
            val kingRoute =
                BitboardUtils.getHorizontalLine(kingOrigin, kingDestiny) and kingOrigin.inv()
    ...
    fun canCastleQueenSide(color: Int, ai: AttacksInfo): Long {
        if (if (color == Color.W) whiteQueensideCastling else blackQueensideCastling) {
            ...
            val rookRoute =
                BitboardUtils.getHorizontalLine(rookOrigin, rookDestiny) and rookOrigin.inv()
            val kingOrigin = kings and if (color == Color.W) whites else blacks
            val kingDestiny = CASTLING_KING_DESTINY_SQUARE[if (color == Color.W) 1 else 3]
            val kingRoute =
                BitboardUtils.getHorizontalLine(kingDestiny, kingOrigin) and kingOrigin.inv()
    ...
    private fun getPieceUnicodeAt(square: Long): Char {
    ...
    fun setupWithFen(fen: String, engine: SearchEngine) {
        this.fen = fen
        engine.go()
    }
    /**
     ...
     */
    private fun setFenMove(fen: String, lastMove: String?) {
        ...
        var i = 0
        var j = Square.A8
        val tokens = fen.split("[ \\t\\n\\x0B\\f\\r]+".toRegex()).dropLastWhile(String::isEmpty)
            .toTypedArray()
        val board = tokens[0]
        ...
        while (i < board.length && j != 0L) {
            ...
            if (p != '/') {
                ...
                for (k in 0 until (if (number == 0) 1 else number)) {
                    tmpWhites =
                        tmpWhites and j.inv() or if (number == 0 && p == p.uppercaseChar()) j else 0
                    tmpBlacks =
                        tmpBlacks and j.inv() or if (number == 0 && p == p.lowercaseChar()) j else 0
        ...
        if (tokens.size > 2) {
            ...
            val whiteKingLateralSquares = longArrayOf(
                BitboardUtils.b_d and (tmpKings and tmpWhites) - 1,
                BitboardUtils.b_d and ((tmpKings and tmpWhites) - 1 or (tmpKings and tmpWhites)).inv(),
                BitboardUtils.b_u and (tmpKings and tmpBlacks) - 1,
                BitboardUtils.b_u and ((tmpKings and tmpBlacks) - 1 or (tmpKings and tmpBlacks)).inv()
            )

            // Squares where we can find a castling rook
            val possibleCastlingRookSquares = longArrayOf(0, 0, 0, 0)

            for (k in 0 until castlings.length) {
            ...
            tmpCastlingRooks[0] =
                BitboardUtils.lsb(tmpRooks and tmpWhites and possibleCastlingRookSquares[0])
            tmpCastlingRooks[1] =
                BitboardUtils.msb(tmpRooks and tmpWhites and possibleCastlingRookSquares[1])
            tmpCastlingRooks[2] =
                BitboardUtils.lsb(tmpRooks and tmpBlacks and possibleCastlingRookSquares[2])
            tmpCastlingRooks[3] =
                BitboardUtils.msb(tmpRooks and tmpBlacks and possibleCastlingRookSquares[3])
            ...
            if (tokens.size > 3) {
                ...
                if (tokens.size > 4) {
                    ...
                    if (tokens.size > 5) {
                        ...
                        fenMoveNumber =
                            ((if (aux > 0) aux - 1 else aux) shl 1) + if (tmpFlags and FLAG_TURN == 0L) 0 else 1
                        if (fenMoveNumber < 0) {
                            fenMoveNumber = 0
                        }
                    }
    ...
    private fun resetHistory() {
        platformUtils.arrayFill(whitesHistory, 0)
        platformUtils.arrayFill(blacksHistory, 0)
        platformUtils.arrayFill(pawnsHistory, 0)
        platformUtils.arrayFill(knightsHistory, 0)
        platformUtils.arrayFill(bishopsHistory, 0)
        platformUtils.arrayFill(rooksHistory, 0)
        platformUtils.arrayFill(queensHistory, 0)
        platformUtils.arrayFill(kingsHistory, 0)
        platformUtils.arrayFill(flagsHistory, 0)
        platformUtils.arrayFill(fiftyMovesRuleHistory, 0)
        platformUtils.arrayFill(moveHistory, 0)
        for (i in 0 until MAX_MOVES) {
            platformUtils.arrayFill(keyHistory[i], 0)
        }
        movesSan.clear()
    }
    ...
    fun doMove(move: Int, verifyCheck: Boolean = true, fillSanInfo: Boolean = true): Boolean {
        ...
        if (flags and FLAGS_PASSANT != 0L) {
            ...
            key[1 - color] =
                key[1 - color] xor ZobristKey.passantFile[BitboardUtils.getFile(flags and FLAGS_PASSANT)]
            // and from the flags
        ...
        var moveMask =
            from or to // Move is as easy as xor with this mask (exceptions are promotions, captures and en-passant captures)
        ...
        if (capture) {
            ...
            key[1 - color] =
                key[1 - color] xor ZobristKey.getKeyPieceIndex(toIndexCapture, getPieceAt(to))
        ...
        when (pieceMoved) {
            Piece.PAWN -> {
                ...
                if (flags and FLAGS_PASSANT != 0L) {
                    key[color] =
                        key[color] xor ZobristKey.passantFile[BitboardUtils.getFile(flags and FLAGS_PASSANT)]
                }
                ...
                if (moveType == Move.TYPE_PROMOTION_QUEEN || moveType == Move.TYPE_PROMOTION_KNIGHT || moveType == Move.TYPE_PROMOTION_BISHOP
                    ...
                    key[color] =
                        key[color] xor (ZobristKey.pawn[color][fromIndex] xor ZobristKey.pawn[color][toIndex])
                }
            }
            ...
            Piece.ROOK -> {
                ...
                key[color] =
                    key[color] xor (ZobristKey.rook[color][fromIndex] xor ZobristKey.rook[color][toIndex])
            }

            Piece.BISHOP -> {
                bishops = bishops xor moveMask
                key[color] =
                    key[color] xor (ZobristKey.bishop[color][fromIndex] xor ZobristKey.bishop[color][toIndex])
            }

            Piece.KNIGHT -> {
                knights = knights xor moveMask
                key[color] =
                    key[color] xor (ZobristKey.knight[color][fromIndex] xor ZobristKey.knight[color][toIndex])
            }

            Piece.QUEEN -> {
                queens = queens xor moveMask
                key[color] =
                    key[color] xor (ZobristKey.queen[color][fromIndex] xor ZobristKey.queen[color][toIndex])
            }
            ...
            -> {
                if (moveType == Move.TYPE_KINGSIDE_CASTLING || moveType == Move.TYPE_QUEENSIDE_CASTLING) {
                    ...
                    key[color] =
                        key[color] xor (ZobristKey.rook[color][originRookIndex] xor ZobristKey.rook[color][destinyRookIndex])
                ...
                key[color] =
                    key[color] xor (ZobristKey.king[color][fromIndex] xor ZobristKey.king[color][toIndex])
            }
        }
    ...
    val isDraw: Boolean
        get() {
            ...
            return pawns == 0L && rooks == 0L && queens == 0L && (bishops == 0L && knights == 0L
                    ...
                    || bishops == 0L && (BitboardUtils.popCount(knights) == 1 || BitboardUtils.popCount(
                knights
            ) == 2 && (BitboardUtils.popCount(knights and whites) == 2 || BitboardUtils.popCount(
                knights and whites.inv()
            ) == 2))
                    || knights == 0L
    ...
    private fun see(move: Int): Int {
        return see(
            Move.getFromIndex(move),
            Move.getToIndex(move),
            Move.getPieceMoved(move), if (Move.isCapture(
                    move
                )
    ...
    fun see(move: Int, ai: AttacksInfo): Int {
        ...
        if (ai.boardKey == getKey()
            && ai.attackedSquares[them] and Move.getToSquare(
                move
            ) == 0L
            && ai.mayPin[them] and Move.getFromSquare(move) == 0L
        ) {
            return if (Move.isCapture(move)) SEE_PIECE_VALUES[Move.getPieceCaptured(
                this,
                move
            )] else 0
        } else {
    ...
    fun getLegalMove(move: Int): Int {
        generateLegalMoves()
        for (i in 0 until legalMoveCount) {
    ...
    val moves: String
        get() {
            ...
            for (i in initialMoveNumber until moveNumber) {
                if (oSB.isNotEmpty()) {
                    oSB.append(" ")
                }
    ...
    companion object {
        ...
        const val FEN_START_POSITION = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"

        // Flags: must be changed only when moving
        private const val FLAG_TURN = 0x0001L
        private const val FLAG_WHITE_KINGSIDE_CASTLING = 0x0002L
        private const val FLAG_WHITE_QUEENSIDE_CASTLING = 0x0004L
        private const val FLAG_BLACK_KINGSIDE_CASTLING = 0x0008L
        private const val FLAG_BLACK_QUEENSIDE_CASTLING = 0x0010L
        private const val FLAG_CHECK = 0x0020L

        // Position on boarch in which is captured
        private const val FLAGS_PASSANT = 0x0000ff0000ff0000L
