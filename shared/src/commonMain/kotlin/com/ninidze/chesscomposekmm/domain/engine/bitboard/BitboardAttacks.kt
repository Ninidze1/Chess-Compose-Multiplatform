package com.ninidze.chesscomposekmm.domain.engine.bitboard
 ...
 */
open class BitboardAttacks internal constructor(): KoinComponent {
    ...
    private fun squareAttackedAux(square: Long, shift: Int, border: Long): Long {
        var s = square
        if (s and border == 0L) {
            if (shift > 0) {
                s = s shl shift
    ...
    private fun squareAttackedAuxSlider(square: Long, shift: Int, border: Long): Long {
        var s = square
        var ret: Long = 0
    ...
    init {
        ...
        while (square != 0L) {
            ...
            bishop[i] =
                squareAttackedAuxSlider(square, +9, BitboardUtils.b_u or BitboardUtils.b_l) or
                        squareAttackedAuxSlider(
                            square,
                            +7,
                            BitboardUtils.b_u or BitboardUtils.b_r
                        ) or
                        squareAttackedAuxSlider(
                            square,
                            -7,
                            BitboardUtils.b_d or BitboardUtils.b_l
                        ) or
     ...
     */
    fun isSquareAttacked(board: com.ninidze.chesscomposekmm.domain.engine.Board, square: Long, white: Boolean): Boolean {
     ...
     */
    private fun isIndexAttacked(board: com.ninidze.chesscomposekmm.domain.engine.Board, index: Int, white: Boolean): Boolean {
        if (index < 0 || index > 63) {
            return false
        }
        ...
        if (pawn[if (white) Color.W else Color.B][index] and board.pawns and others != 0L) {
            ...
            return true
        } else if (getBishopAttacks(
                index,
                all
            ) and (board.bishops or board.queens) and others != 0L
        ) {
            return true
        }
        return false
     ...
     * Discover attacks to squares using magics: cheap version
     */
    fun getIndexAttacks(board: com.ninidze.chesscomposekmm.domain.engine.Board, index: Int): Long {
        if (index < 0 || index > 63) {
            return 0
        }
        val all = board.all

        return board.blacks and pawn[Color.W][index] or (board.whites and pawn[Color.B][index]) and board.pawns or (king[index] and board.kings) or (knight[index] and board.knights) or (getRookAttacks(
            index,
            all
        ) and (board.rooks or board.queens)) or (getBishopAttacks(
            index,
            all
        ) and (board.bishops or board.queens))
    }

    fun getXrayAttacks(board: com.ninidze.chesscomposekmm.domain.engine.Board, index: Int, all: Long): Long {
        if (index < 0 || index > 63) {
            return 0
        }
        return getRookAttacks(index, all) and (board.rooks or board.queens) or (getBishopAttacks(
            index,
            all
        ) and (board.bishops or board.queens)) and all
    }
    ...
    fun getRookShiftAttacks(square: Long, all: Long): Long {
        return checkSquareAttackedAux(square, all, +8, BitboardUtils.b_u) or checkSquareAttackedAux(
            square, all, -8,
            BitboardUtils.b_d
        ) or checkSquareAttackedAux(square, all, -1, BitboardUtils.b_r) or checkSquareAttackedAux(
            square, all, +1,
            BitboardUtils.b_l
        )
    }
    ...
    fun getBishopShiftAttacks(square: Long, all: Long): Long {
        return checkSquareAttackedAux(
            square,
            all,
            +9,
            BitboardUtils.b_u or BitboardUtils.b_l
        ) or checkSquareAttackedAux(
            square,
            all,
            +7,
            BitboardUtils.b_u or BitboardUtils.b_r
            ...
            -7,
            BitboardUtils.b_d or BitboardUtils.b_l
        ) or checkSquareAttackedAux(square, all, -9, BitboardUtils.b_d or BitboardUtils.b_r)
    }
    ...
    companion object {
         ...
         */
        private var USE_MAGIC = true
        private var instance: BitboardAttacks? = null
