package com.ninidze.chesscomposekmm.domain.engine.tt
 ...
 * @author rui
 */
class TranspositionTable(sizeMb: Int): KoinComponent{

    private var keys: LongArray
    private var infos: LongArray
    private var evals: ShortArray
    ...
    fun search(board: com.ninidze.chesscomposekmm.domain.engine.Board, distanceToInitialPly: Int, exclusion: Boolean): Boolean {
        info = 0
        score = 0
    ...
    operator fun set(board: com.ninidze.chesscomposekmm.domain.engine.Board, nodeType: Int, distanceToInitialPly: Int, depthAnalyzed: Int, bestMoveInVal: Int, scoreInVal: Int, eval: Int, exclusion: Boolean) {
...
//        assert(Math.abs(eval) < SearchEngine.VALUE_IS_MATE || Math.abs(eval) == Evaluator.MATE || eval == Evaluator.NO_VALUE) { "Storing a eval value in the TT outside limits" }
    ...
    companion object {
        ...
        const val DEPTH_QS_CHECKS = 0
        const val DEPTH_QS_NO_CHECKS = -1

        const val TYPE_EVAL = 0
        const val TYPE_EXACT_SCORE = 1
        const val TYPE_FAIL_LOW = 2
        const val TYPE_FAIL_HIGH = 3
