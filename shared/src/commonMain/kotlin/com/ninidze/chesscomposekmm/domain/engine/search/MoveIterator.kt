...
class MoveIterator(private var searchEngine: SearchEngine?, private val ai: AttacksInfo, private val depth: Int) {
    ...
    private var board: com.ninidze.chesscomposekmm.domain.engine.Board
     ...
     */
    private fun generateNonCaptures() {
    ...
    private fun generateCheckEvasionCaptures() {
    ...
    private fun generateCheckEvasionsNonCaptures() {
    ...
    private fun addMove(pieceMoved: Int, fromIndex: Int, from: Long, to: Long, capture: Boolean, moveType: Int) {
        ...
        if (moveType == Move.TYPE_KINGSIDE_CASTLING || moveType == Move.TYPE_QUEENSIDE_CASTLING) {
            ...
            newMyKingIndex = com.ninidze.chesscomposekmm.domain.engine.Board.CASTLING_KING_DESTINY_INDEX[j]
            // Castling has a special "to" in Chess960 where the destiny square is the rook
            val kingTo = com.ninidze.chesscomposekmm.domain.engine.Board.CASTLING_KING_DESTINY_SQUARE[j]
            val rookTo = com.ninidze.chesscomposekmm.domain.engine.Board.CASTLING_ROOK_DESTINY_SQUARE[j]
        ...
        if (capture || movesToGenerate == GENERATE_CAPTURES_PROMOS_CHECKS && check) {
            ...
            if (ai.attackedSquares[them] and to == 0L && ai.mayPin[them] and from == 0L) {
                see = if (capture) com.ninidze.chesscomposekmm.domain.engine.Board.SEE_PIECE_VALUES[pieceCaptured] else 0
            } else {
    ...
    companion object {
        ...
        const val GENERATE_ALL = 0
        const val GENERATE_CAPTURES_PROMOS = 1 // Generates only good/equal captures and queen promotions
        const val GENERATE_CAPTURES_PROMOS_CHECKS = 2 // Generates only good/equal captures, queen promotions and checks
        //
        // Move generation phases
        //
        const val PHASE_TT = 0
        const val PHASE_GEN_CAPTURES = 1
        const val PHASE_GOOD_CAPTURES_AND_PROMOS = 2
        const val PHASE_EQUAL_CAPTURES = 3
        const val PHASE_GEN_NON_CAPTURES = 4
        const val PHASE_KILLER1 = 5
        const val PHASE_KILLER2 = 6
        const val PHASE_KILLER3 = 7
        const val PHASE_KILLER4 = 8
        const val PHASE_NON_CAPTURES = 9
        const val PHASE_BAD_CAPTURES = 10
        const val PHASE_END = 11
        ...
        private const val SCORE_PROMOTION_QUEEN = 975
        private const val SCORE_UNDERPROMOTION = Int.MIN_VALUE + 1
        private const val SCORE_LOWEST = Int.MIN_VALUE

        const val SEE_NOT_CALCULATED = Short.MAX_VALUE.toInt()
    }
}