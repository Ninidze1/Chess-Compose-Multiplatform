...
object Move {
    ...
    private const val PIECE_LETTERS_LOWERCASE = " pnbrqk"
    private const val PIECE_LETTERS_UPPERCASE = " PNBRQK"

    // Move Types
    const val TYPE_KINGSIDE_CASTLING = 1
    const val TYPE_QUEENSIDE_CASTLING = 2
    const val TYPE_PASSANT = 3
    // Promotions must be always >= TYPE_PROMOTION_QUEEN
    const val TYPE_PROMOTION_QUEEN = 4
    const val TYPE_PROMOTION_KNIGHT = 5
    const val TYPE_PROMOTION_BISHOP = 6
    const val TYPE_PROMOTION_ROOK = 7
    const val TYPE_PROMOTION_KING = 8 // For giveaway/suicide variants

    const val CHECK_MASK = 0x1 shl 16
    private const val CAPTURE_MASK = 0x1 shl 15
     ...
     */
    private fun isPromotion(move: Int): Boolean {
        return getMoveType(move) >= TYPE_PROMOTION_QUEEN
    }

    private fun getPiecePromoted(move: Int): Int {
    ...
    fun getFromString(board: Board, move: String, verifyValidMove: Boolean): Int {
        ...
        val toIndex: Int = BitboardUtils.algebraic2Index(m.substring(m.length - 2, m.length))
    ...
    fun toSan(board: Board, move: Int): String {
        ...
        var fileEqual = false
        var rankEqual = false
        for (i in 0 until board.legalMoveCount) {
